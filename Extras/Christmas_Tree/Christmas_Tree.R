############## Minha àrvore em R <3 ##############
### Adaptada de Áquila Delfino, linkedin: https://www.linkedin.com/in/aquiladelfino/
...
chtree <- function(bulbs = TRUE, snow = TRUE, star = TRUE, bg.col = 1)
  ...
  plot(0, xlim = c(-1.25, 1.25), axes = F, xlab = "", ylab = "", ylim = c(-100, 650))
  a <- 0
  
  rect(-0.1, -100, 0.1, 0, col = "brown", border = "brown")
  
  title(main = "FELIZ NATAL!!!", 
        font.main = 2,
        line = -1,
        col.main = "red")
  ...
  if(isTRUE(star)) {
    ...
    for(i in 11:1){
      points(0.005, 500.2, pch = 19, col = rgb(1, 1, 0, 0.05), cex = 3 + (0.5 * i))
    }
  }
  ...
  if(isTRUE(star)) {
    # Star
    points(0.006, 508, pch = 19, col = 7, cex = 6)
    p(0, 520, pch = "⭐", cex = 12, col = 7)
  }
